...
// https://youtu.be/u-DEw481l04?si=PqM5BddZeOSxkE-M

// one by one har character ki freq ko base freq leke uske liye deletions nikal lo  and sabme se min lelo
// to find deletions:
// for all other characters:
// If freq > f + k ⇒ delete extra
// If freq < f ⇒ delete entire character 
...
public:
    int minimumDeletions(string word, int k) {
        ...
        for(int i = 0; i < 26; i++) {
            ...
            for(int j = 25; j > i; j--) { //find deletion for j = 25 to j > i
                if(freq[j] - freq[i] <= k) 
                    break;
