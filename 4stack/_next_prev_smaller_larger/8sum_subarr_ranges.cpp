...
// m1 bruteforcee
// sab subarray nikalke krlo

//m2
//helper ques - lc 907 - sum of subbarray minimums
...
// ans= sum of subbarray minimums - sum of subarray maximums 
    ...
    vector<int> nextSmaller(vector<int>arr, int n)
{
 ...
 vector<int> prevBig(vector<int>arr, int n)
    {
        stack<int> st;
        vector<int> ans(n);
        for (int i = 0; i < n; i++)
        {
            int curr = arr[i];
            while (!st.empty()&&arr[st.top()] < curr) //strictly lesser to avoid duplicate addition
            {
                st.pop();
            }
    ...
    vector<int> nextBig(vector<int>arr, int n)
{
    stack<int> st;
    vector<int> ans(n);
    for (int i = n - 1; i >= 0; i--)
    {
        int curr = arr[i];
        while (!st.empty()&&arr[st.top()] <= curr) //lesser or equal
        {
            st.pop();
        }
        ans[i]=st.empty()?n:st.top();
    ...
    long long subArrayRanges(vector<int>& nums) {
        int n=nums.size();
        vector<int>prevsmall=prevSmaller(nums,n);
        vector<int>nextsmall=nextSmaller(nums,n);
        vector<int>nextbig=nextBig(nums,n);
        vector<int>prevbig=prevBig(nums,n);

        long long ansSmall=0;
        long long ansBig=0;
        ...
        for(int i=0;i<nums.size();i++){
            ...
            long long left=i-prev;
            long long right=next-i;
            long long contri=(left*right)*nums[i];
            ansSmall+=contri;;
        }
        for(int i=0;i<nums.size();i++){
            int prev=prevbig[i];
